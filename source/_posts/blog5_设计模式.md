...
### 1.创建型模式
* 核心作用：<font color=red>创建对象</font>(关注对象的创建过程)
...
### 2.结构型模式
* 核心作用：从程序结构上实现解耦，扩大整体类结构，解决更大的问题(关注<font color=red>类和对象</font>的组织)
...
* 静态代理：静态定义代理类（有一个类文件描述代理模式）
* 动态代理：动态生成代理类（在内存中形成代理类）
	* 实现方法
		* JDK自带的动态代理
...
* 核心
	...
	* 遵循迪米特法则，一个类尽可能少地与其他类打交道，本质上就是对复杂的类关系进行封装，提供统一的调用方法
* 使用场景
	* 几乎随处可见，各种技术和框架中都有外观模式的使用
	* 如jdbc封装后commons提供的DBUtils工具类


#### 2.7享元模式
* 核心
	* 以共享的方式高效地支持大量细粒度对象的重用
	* 实现共享的关键是区分了内部状态和外部状态
		* 内部状态：可以共享，不会随环境变化而变化
		* 外部状态：不可以共享，会随环境变化而改变
* 场景
	* 对多个完全相同或相似的对象使用享元模式，实现节省内存的目的
	* 由于其共享的特性，可以在任何"池"中操作，如线程池、数据库连接池等，String类(常量池)的设计也是享元模式
* 实现
	* 享元工厂类
		* 创建并管理享元对象，享元池一般被设计成键值对
	* 抽象享元类
		* 通常是一个接口或抽象类，声明公共方法，对外提供对象的内部状态，设置外部状态
	* 具体享元类
		* 为内部状态提供成员变量进行存储
	* 非共享享元类
		* 不能不被共享的子类可以设计为非共享享元类
* 优点
	* 极大减少内存中的对象数量，节约资源，提高系统性能
	* 外部状态相对独立，不影响内部状态
* 缺点
	* 使程序逻辑复杂化
	* 为了节约空间，共享了内部状态，分离出外部状态，而读取外部状态使运行时间变长。用时间换取了空间。

### 3.行为型模式
* 关注系统中<font color=red>对象与对象</font>的交互，研究系统运行时对象间的通信和协作，明确对象的职责

#### 3.1责任链模式
* 核心
	* 将能处理同一类请求的对象连成一条链，所提交的请求沿着链传递，链上的对象逐个判断是否又处理该请求的能力，能处理则处理，不能处理则传递给链上的下一个对象
